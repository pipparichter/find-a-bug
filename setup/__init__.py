...
def download_gtdb_gtdb_version(gtdb_version:int, ):
    ...
    os.mkdir(os.path.join(gtdb_version_dir, 'amino_acid_seqs'))
    ...
    with tarfile.open(os.path.join(gtdb_version_dir, f'gtdb_proteins_aa_reps_r{gtdb_version}.tar.gz'), 'rb') as f:
        f.extractall(os.path.join(gtdb_version_dir, 'proteins'))
    # Rename all protein files to match convention...
    for file_name in os.listdir(os.path.join(gtdb_version_dir, 'proteins', 'bacteria')):
        source_path = os.path.join(gtdb_version_dir, 'proteins', 'bacteria', file_name)
        dest_path = os.path.join(gtdb_version_dir, 'proteins', 'bacteria', file_name.replace('_protein', ''))
        os.rename(source_path, dest_path)
    for file_name in os.listdir(os.path.join(gtdb_version_dir, 'proteins', 'archaea')):
        source_path = os.path.join(gtdb_version_dir, 'proteins', 'bacteria', file_name)
        dest_path = os.path.join(gtdb_version_dir, 'proteins', 'bacteria', file_name.replace('_protein', ''))
        os.rename(source_path, dest_path)
